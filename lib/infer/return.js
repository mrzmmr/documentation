  ...
  return function inferReturn(comment) {
    types.visit(comment.context.ast, {
      visitFunction: function (path) {
        if (path.value.returnType && path.value.returnType.typeAnnotation) {
          if (!comment.returns) {
            comment.returns = [{
              type: flowDoctrine(path.value.returnType.typeAnnotation)
            }];
          } else if (comment.returns.length === 1 && !comment.returns[0].type) {
            comment.returns[0].type = flowDoctrine(path.value.returnType.typeAnnotation);
          }
        }
